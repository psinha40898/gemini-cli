...
describe('parseInputForHighlighting', () => {
  it('should handle an empty string', () => {
    expect(parseInputForHighlighting('', 0, [], undefined)).toEqual([
      { text: '', type: 'default' },
    ]);
  });
  ...
  it('should handle text with no commands or files', () => {
    ...
    expect(parseInputForHighlighting(text, 0, [], undefined)).toEqual([
      { text, type: 'default' },
    ]);
  });
  ...
  it('should NOT highlight a command at the beginning when index is not 0', () => {
    const text = '/help me';
    expect(parseInputForHighlighting(text, 1, [], undefined)).toEqual([
  ...
  it('should not highlight command but highlight file on a non-zero line', () => {
    ...
    expect(parseInputForHighlighting(text, 2, [], undefined)).toEqual([
