...
export interface SettingsDialogState {
  ...
  unsavedRestartChanges: Map<string, PendingValue>;
}
...
export function settingsDialogReducer(
  ...
  switch (action.type) {
    ...
    case 'ADD_PENDING_CHANGE': {
      const next = new Map(state.unsavedRestartChanges);
      next.set(action.key, action.value);
      return { ...state, unsavedRestartChanges: next };
    }

    case 'REMOVE_PENDING_CHANGE': {
      if (!state.unsavedRestartChanges.has(action.key)) return state;
      const next = new Map(state.unsavedRestartChanges);
    ...
    case 'SAVE_AND_CLEAR_KEYS': {
      if (state.unsavedRestartChanges.size === 0) return state;
      const next = new Map(state.unsavedRestartChanges);
    ...
    case 'CLEAR_ALL_PENDING':
      return { ...state, unsavedRestartChanges: new Map() };
