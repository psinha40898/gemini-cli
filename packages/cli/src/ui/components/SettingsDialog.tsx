...
export function SettingsDialog({
  ...
  // Define the item type for ScrollableList
  ...
  // Calculate the height for ScrollableList (each item is 2 rows: content + spacing)
  const scrollableHeight = effectiveMaxItemsToShow * 2;

  // Scroll to keep activeSettingIndex visible when it changes
  useEffect(() => {
    ...
    (key) => {
      ...
      if (focusSection === 'settings') {
        ...
        if (keyMatchers[Command.DIALOG_NAVIGATION_UP](key)) {
          ...
          // Scrolling is handled by ScrollableList via scrollToIndex
